...
public sealed class TestCsvMetadata : IDisposable
{
    private readonly string testDataDirectory;
    private readonly string validCsvPath;

    public TestCsvMetadata()
    {
...
123,/test/data/test-case4.pdf,.pdf,2025-01-18 12:00:00,IA/2025/004,UKUT-IAC,Brown,Home Office,Immigration,Entry Clearance,Administrative Law,Case Management,Duplicate ID case";
    ...
    {
        // Act
        var lines = CsvMetadata.Read(validCsvPath);
        var result = CsvMetadata.FindLines(lines, 123);
    ...
    public void FindLines_WithValidIdSingleMatch_ReturnsSingleLine()
    {
        // Act
        var lines = CsvMetadata.Read(validCsvPath);
        var result = CsvMetadata.FindLines(lines, 124);
    ...
    public void FindLines_WithNonExistentId_ReturnsEmptyList()
    {
        // Act
        var lines = CsvMetadata.Read(validCsvPath);
        var result = CsvMetadata.FindLines(lines, 999);
    ...
    public void FindLines_WithZeroId_ReturnsEmptyList()
    {
        // Act
        var lines = CsvMetadata.Read(validCsvPath);
        var result = CsvMetadata.FindLines(lines, 0);
    ...
    public void Read_WithNonExistentFile_ThrowsFileNotFoundException()
    {
        ...
        // Act & Assert
        Assert.Throws<FileNotFoundException>(() => { _ = CsvMetadata.Read(nonExistentPath); });
    }
    ...
    public void FindLines_WithEmptyFile_ReturnsEmptyList()
    {
        ...
        // Act
        var lines = CsvMetadata.Read(emptyCsvPath);
        var result = CsvMetadata.FindLines(lines, 123);
    ...
    [Fact]
    public void Read_WithMalformedCsv_ThrowsCsvHelperException()
    {
...
124,2025-01-16 10:00:00,Jones,HMRC";
        ...
        var ex = Assert.Throws<CsvHelper.MissingFieldException>(() => { _ = CsvMetadata.Read(malformedCsvPath); });
    ...
    [Fact]
    public void Read_PartiallyMissingRequiredColumns_ThrowsCsvHelperException()
    {
...
123,.pdf,2025-01-15 09:00:00,IA/2025/001,Smith,Secretary of State for the Home Department,UKUT-IAC";
        ...
        var ex = Assert.Throws<CsvHelper.MissingFieldException>(() => { _ = CsvMetadata.Read(partialCsvPath); });
    ...
    [Fact]
    public void Read_WithMainSubcategoryButNoMainCategory_ThrowsCsvValidationException()
    {
...
126,/test/data/test-case-invalid.pdf,.pdf,2025-01-15 09:00:00,IA/2025/001,UKUT-IAC,Smith,Secretary of State,,Appeals,Tax,VAT,Test case with orphaned main_subcategory";
        ...
        var ex = Assert.Throws<CsvHelper.CsvHelperException>(() => { _ = CsvMetadata.Read(invalidCategoryCsvPath); });
    ...
    [Fact]
    public void Read_WithSecSubcategoryButNoSecCategory_ThrowsCsvValidationException()
    {
...
127,/test/data/test-case-invalid2.pdf,.pdf,2025-01-15 09:00:00,IA/2025/001,UKUT-IAC,Smith,Secretary of State,Immigration,Appeals,,VAT,Test case with orphaned sec_subcategory";
        ...
        // Act & Assert - Should throw CsvHelperException during CSV reading
        var ex = Assert.Throws<CsvHelper.CsvHelperException>(() => { _ = CsvMetadata.Read(invalidSecCategoryCsvPath); });
    ...
    [Fact]
    public void Read_WithValidCategoryHierarchy_ProcessesSuccessfully()
    {
...
128,/test/data/test-case-valid.pdf,.pdf,2025-01-15 09:00:00,IA/2025/001,UKUT-IAC,Smith,Secretary of State,Immigration,Appeals,Tax,VAT,Test case with valid category hierarchy";
        ...
        var lines = CsvMetadata.Read(validCategoryCsvPath);
