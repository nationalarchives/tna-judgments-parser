...
public sealed class BacklogParserOptions
{
    ...
    [Required] public required string CourtMetadataFilePath { get; set; }

    [Required] public required string DataFolderPath { get; set; }

    [Required] public required string OutputFolderPath { get; set; }

    [Required] public required string TrackerFilePath { get; set; }
...
public class BacklogParserOptionsValidation(IFileSystem fileSystem) : IValidateOptions<BacklogParserOptions>
{
    public ValidateOptionsResult Validate(string? name, BacklogParserOptions options)
    {
        ...
        if (!string.IsNullOrWhiteSpace(options.CourtMetadataFilePath) &&
            !fileSystem.File.Exists(options.CourtMetadataFilePath))
        {
            validationResults.Add(new ValidationResult(
