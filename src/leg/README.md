...
When the parser output format changes, the expected test files can be regenerated. The
regenerators (`RegenerateAllTestFiles` for `.akn`, `RegenerateAllHtml` for `.html`) are gated
behind the `UPDATE_XML` environment variable so that a normal `dotnet test` run never rewrites
fixtures; they only run when `UPDATE_XML="true"` is passed. The `.akn` regenerators also skip
writing any file whose only differences are non-deterministic metadata (FRBRdate dates, the
`<ukm:Parser>` version, `<uk:hash>`), so a regeneration produces a minimal, meaningful diff.

Run the regenerator for the relevant document type, e.g.:
