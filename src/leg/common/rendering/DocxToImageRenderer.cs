...
// Shared, transport-agnostic worker that drives `soffice` to convert a DOCX to
// HTML and pulls drawing images back out by marker. Used by:
//   - LocalSubprocessRenderer (parser-side, dev PCs)
//   - RenderService (HTTP service running on a LibreOffice EC2)
// Single source of truth so dev fixtures and EC2 output don't diverge.
public sealed class DocxToImageRenderer {

    private static readonly ILogger logger =
        Logging.Factory.CreateLogger<DocxToImageRenderer>();
