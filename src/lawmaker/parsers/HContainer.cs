...
public partial class LegislationParser
{
    ...
    private HContainer ParseLine(System.Func<WLine, HContainer> nextExpected = null)
    {
        ...
        Logger.LogWarning("Encountered an unknown provision at " +
    ...
    */
    private IList<IBlock> GetExtraParagraph(WLine leader, System.Func<WLine, HContainer> nextExpected = null)
    {
        ...
        if (next is UnnumberedLeaf)
        {
            ...
            var numberText = GetScheduleNumber(leader, true);
            var isScheduleNumber = LanguageService.IsMatch(numberText, Schedule.NumberPatterns);
    ...
    private bool BreakFromProv1()
    {
        ...
        {
            var parentNum = GetNumString(provisionRecords.CurrentNumber(quoteDepth));
            var childNum = GetNumString(line);
    ...
    private static bool IsSubsequentAlphanumeric(string lo, string hi)
    {
        ...
        if (hi.Length < lo.Length)
        {
            ...
            // i.e. (1B -> 2, 1CA -> 1D)
            var loTrimmed = string.Concat(lo.SkipLast(1));
            ...
            // 5ZC -> 5A, AZ3 -> A1
            var diff = Regex.Replace(hi, @$"^{loTrimmed}", "");
    ...
    private static string IncrementAlphanumeric(string numString)
    {
        ...
        // For example: 1 -> 2
        var isInt = int.TryParse(numString, out var numInt);
        if (isInt)
            return (numInt + 1).ToString();

        // If the num ends with a numeric portion, increment that portion
        // For example: A9 -> A10, 1Z1 -> 1Z2
        var finalDigitsString = Regex.Match(numString, @"\d+$").Value;
        if (finalDigitsString.Length > 0)
        {
            int.TryParse(finalDigitsString, out var finalDigitsInt);
        ...
        // Typical case
        var charIncrement = (char)(finalChar + 1);
    ...
    {
        var isInt = int.TryParse(numString, out _);
    ...
    private static bool IsSubsequentAlphabetic(string lo, string hi)
    {
        ...
        {
            var loFinalChar = char.ToUpperInvariant(lo.Last());
            var hiFinalChar = char.ToUpperInvariant(hi.Last());
