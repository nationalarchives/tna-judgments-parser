...
public partial class LegislationParser
{
    ...
    private static readonly string quotedStructureInfoPattern = @"(?'info'{(?'docName'.*?)(?:-(?'context'.*?))?}\s*)?";
    ...
    private static string QuotedStructureEndPattern()
    {
        ...
        var followingTextRegex = $"(?'followingText'{string.Join("|", possibleFollowingTexts)})";
...
#nullable enable
    ...
    private bool AddQuotedStructureFrame(string? line)
    {
        ...
        var pattern = $"{quotedStructureInfoPattern}{StartQuotePattern()}";
...
#nullable disable
    ...
    {
        var start = Regex.Matches(text, StartQuotePattern()).Count;
        var end = Regex.Matches(text, EndQuotePattern()).Count;
        return (start, end);
    }
    ...
    private static (int, int) CountStartAndEndQuotes(IEnumerable<IBlock> blocks)
    {
        ...
        {
            (var addToStart, var addToEnd) = CountStartAndEndQuotes(block);
            start += addToStart;
            end += addToEnd;
        }
        return (start, end);
    ...
    {
        var text = Regex.Replace(line.NormalizedContent, @"\s+", string.Empty);
    ...
    {
        QuoteDistance = 0;
        var lastLine = LastLine.GetLastLine(division);
    ...
    {
        QuoteDistance = 0;
        var lastLine = LastLine.GetLastLine(contents);
        if (lastLine == null) return false;
        return IsEndOfQuotedStructure(lastLine, QuoteDistance);
    }
    ...
    private bool IsEndOfQuotedStructure(string text, int quoteDistance)
    {
        ...
        var isEndQuoteAtEnd = Regex.IsMatch(text, QuotedStructureEndPattern());
        if (!isEndQuoteAtEnd)
            return false;

        var isStartQuoteAtStart = Regex.IsMatch(text, QuotedStructureStartPattern());
        (var left, var right) = CountStartAndEndQuotes(text);
        ...
        var isSingleLine = (isStartQuoteAtStart && isEndQuoteAtEnd);
        var isEndOfMultiLine = (!isStartQuoteAtStart && isEndQuoteAtEnd && right > left);
...
#nullable enable
    ...
    private List<IQuotedStructure> HandleQuotedStructuresAfter(WLine line)
    {
        ...
        (var left, var right) = CountStartAndEndQuotes(line);
        var isAtStartOfLine = (left == right + 1) && Regex.IsMatch(line.NormalizedContent, QuotedStructureStartPattern());
        if (left > right && !isAtStartOfLine)
        {
            var isValidFrame = AddQuotedStructureFrame(line);
        ...
        while (i < Body.Count && IsStartOfQuotedStructure(Current()))
        {
            ...
            var isValidFrame = current is not null && AddQuotedStructureFrame(current);
            BlockQuotedStructure? qs = ParseQuotedStructure();
            if (qs == null)
            {
                i = save;
...
#nullable enable
    ...
    private static string? ExtractStartQuote(HContainer hContainer)
    {
        ...
        if (hContainer.HeadingPrecedesNumber && hContainer.Heading is WLine heading)
        {
            ...
            var bookmark = enriched.Contents.First(i => i is WBookmark) as WBookmark;
        ...
        else if (hContainer.Number is not null)
        {
            ...
            if (!match.Success)
                return null;
            var patternEndIndex = match.Index + match.Length;
        ...
        else
        {
            ...
            var startQuote = ExtractStartQuote(line);
    ...
    static IInline? ExtractStartQuoteConstructor(IEnumerable<IInline> inlines)
    {
        ...
        var startQuote = text.Split("}").LastOrDefault();
    ...
    private class ExtractAndReplace
    {
        ...
        */
        public WLine Invoke(WLine line)
        {
            ...
            var lastWText = line.Contents.Last() as WText;
