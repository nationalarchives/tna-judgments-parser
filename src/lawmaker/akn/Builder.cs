...
partial class Builder(Document bill, LanguageService languageService) : AkN.Builder
{
    ...
    protected override string UKNS => "https://www.legislation.gov.uk/namespaces/UK-AKN";
    ...
    private XmlDocument Build()
    {
        ...
        main.SetAttribute("name", bill.Type.ToString().ToLower());
    ...
    private void AddHeader(XmlElement main, IHeader header)
    {
        ...
        else if (header is SPHeader spHeader)
        {
            ...
            {
                main.AppendChild(preface.Build(bill).ToXmlNode(main.OwnerDocument));
            }
        ...
        else if (header is UKHeader ukHeader)
        {
            ...
            {
                main.AppendChild(preamble.Build(bill).ToXmlNode(main.OwnerDocument));
            }
            // ensure our metadata is registered
            if (ukHeader.Title is not null)
            {
                bill.Metadata.Register(new Reference(ReferenceKey.varBillTitle, ukHeader.Title.NormalizedContent));
            }
            if (ukHeader.StageVersion is not null)
            {
                bill.Metadata.Register(new Reference(ReferenceKey.varStageVersion, ukHeader.StageVersion.Stage.ShowAs));
            }
        ...
        else if (header is CMHeader cmHeader)
        {
            ...
            {
                bill.Metadata.Register(new Reference(ReferenceKey.varBillTitle, cmHeader.Title.NormalizedContent));
            }
        }
    ...
    private void AddPreface(XmlElement bill, IList<IBlock> prefaceElements)
    {
        ...
        IEnumerable<XmlElement> elements = prefaceElements
            .Select(block =>
            {
                ...
                var line = block as WLine;
                XmlElement p = doc.CreateElement("p", ns);
                var text = Regex.Replace(line.NormalizedContent, @"\s", "").ToLower();
                switch (text)
                {
                    case "a":
    ...
    private XmlElement TransformPreambleText(XmlElement pElement)
    {
        ...
        var preambleText = pElement.InnerText;
        ...
        var enactingPrefix = preambleText[..ENACTING_PREFIX.Length];
    ...
    private void AddBlocks(XmlElement parent, IEnumerable<IBlock> blocks)
    {
        ...
        foreach (IBlock block in blocks)
        {
            ...
            {
                parent.AppendChild(buildable.Build(bill).ToXmlNode(parent.OwnerDocument));
                continue;
            }
    ...
    protected override void p(XmlElement parent, ILine line)
    {
    ...
    private ILine TrimLine(ILine line)
    {
        ...
        if (newContents.Count() >= 2)
        {
            ...
            if (last is WText text2)
            {
                ...
                var fixedText = Regex.Replace(text2.Text, @"\s*$", "");
                var fixedInline = new WText(fixedText, text2.properties);
                trimmedInlines.Add(fixedInline);
            }
            else
    ...
    {
        foreach (var style in model.Style.Split(";", StringSplitOptions.TrimEntries))
        {
            if (style.Split(":", 2, StringSplitOptions.TrimEntries) is [var key, var val]
                && double.TryParse(Regex.Replace(val, @"[^0-9.-]", ""), out var parsedValue))
            {
    ...
    protected void AddBlockListItem(XmlElement parent, BlockListItem item)
    {
        ...
        if (item.Number is not null)
        {
            ...
            var newNum = new string(item.Number.Text.Select(c => ((uint)c == 61623) ? '\u2022' : c).ToArray());
    ...
    protected void AddSigBlock(XmlElement parent, ISignatureBlock sig)
    {
        ...
        if (sig.Name.Equals("date"))
        {
            ...
            var text = (sig.Content.First() as WText).Text;
            ...
            foreach (CultureInfo culture in languageService.Cultures)
            {
                ...
                var parsedDate = DateTime.TryParseExact(text, "d MMMM yyyy", culture, DateTimeStyles.None, out DateTime dateTime);
                if (parsedDate)
                {
