...
partial class Numbering3
{
    ...
    private static int CalculateN(NumberingContext ctx, Paragraph paragraph, int ilvl)
    {
        ...
        if (ctx.TryGetCachedN(paragraph, ilvl, out var cached))
            return cached;
        return 0;
    }
        ...
        int currentIlvl)
    {
        ...
        var currentStyleId = currentStyle.StyleId?.Value;
        if (currentStyleId == targetStyleId)
            return true;

        var basedOn = currentStyle.BasedOn?.Val?.Value;
        while (basedOn != null)
        {
    ...
    private static void CalculateAllNumbers(NumberingContext ctx)
    {
        ...
        bool ShouldApplyOverride(int absNumId, int numId, int ilvl, bool requiresParentOwner, bool isGloballyNew, out int overrideVal)
        {
            ...
            if (requiresParentOwner && ilvl > 0 &&
                levelOwners.TryGetValue((absNumId, ilvl - 1), out var parentOwner) && parentOwner != numId)
            {
                overrideVal = default;
                return false;
            }
            ...
            var hasExplicit = HasExplicitOverride(ctx.Main, numId, ilvl);
            var justCameFromDeeperWithOverride = (!isGloballyNew || !hasExplicit) &&
                                                   lastIlvls.TryGetValue(absNumId, out var prevIlvl) &&
                                                   prevIlvl > ilvl &&
        ...
        foreach (var paragraph in ctx.Main.Document.Body.Descendants<Paragraph>())
        {
            ...
            var ownNumId = paragraph.ParagraphProperties?.NumberingProperties?.NumberingId?.Val?.Value;
            var hasExplicitNumId = ownNumId.HasValue;
            // TODO consider paragraph.ParagraphProperties?.NumberingProperties?.NumberingChange?.Id?.Value
            var styleNumId = Styles.GetStyleProperty(style, s => s.StyleParagraphProperties?.NumberingProperties?.NumberingId?.Val?.Value);
            var numIdOpt = ownNumId ?? styleNumId;
            if (!numIdOpt.HasValue)
                continue;
            var numId = numIdOpt.Value;

            int? ownIlvl;

            if (numId == 0)
            {
                if (TryListNum(paragraph, out var listNumId, out var listIlvl))
                {
                    ownNumId = listNumId;
                    numId = listNumId;
                    ownIlvl = listIlvl;
            ...
            var styleIlvl = Styles.GetStyleProperty(style, s => s.StyleParagraphProperties?.NumberingProperties?.NumberingLevelReference?.Val?.Value);
            var ilvl = ownIlvl ?? styleIlvl ?? 0; // This differs a bit from Numbering.GetNumberingIdAndIlvl

            var requiresParentOwner = !hasExplicitNumId && styleNumId.HasValue;

            if (!numIdToAbsNumId.TryGetValue(numId, out var absNumId))
            {
                var absOpt = GetAbstractNumId(ctx, numId);
                if (!absOpt.HasValue)
                    continue;
            ...
            var isGloballyNew = seenNumIds.Add(numId);
            ...
            for (var parentLevel = 0; parentLevel < ilvl; parentLevel++)
            {
                int parentValue;
            ...
            if (ilvlCounters.TryGetValue(ilvl, out var current))
            {
                ...
                {
                    if (ShouldApplyOverride(absNumId, numId, ilvl, requiresParentOwner, isGloballyNew, out var overrideValue))
            ...
            else
            {
                ...
                var returningFromDeeper = lastIlvls.TryGetValue(absNumId, out var lastIlvl) && lastIlvl > ilvl;
            ...
            var styleId = paragraph.ParagraphProperties?.ParagraphStyleId?.Val?.Value ?? style.StyleId?.Value;
    ...
    private static bool LevelNeverRestarts(MainDocumentPart main, int absNumId, int ilvl)
    {
        ...
        var restart = level?.LevelRestart?.Val?.Value;
        ...
        HashSet<(int numId, int ilvl)> consumed, HashSet<(int absNumId, int ilvl)> overrideConsumedAtLevel, out int value)
    {
        ...
        var startOverride = Numbering2.GetStartOverride(ctx.Main, numId, ilvl);
