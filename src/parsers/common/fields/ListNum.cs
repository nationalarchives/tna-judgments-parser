...
internal class ListNum
{
    ...
    internal static INumber Parse(MainDocumentPart main, Run first, string fieldCode)
    {
        if (fieldCode == " LISTNUM LegalDefault ")
        {
            ...
            var n = CountPrecedingParagraphsWithListNumLegalDefault(p, 1) + 1;
            var num = n.ToString() + ".";
        ...
        if (match.Success)
        {
            ...
            Paragraph p = first.Ancestors<Paragraph>().First();
            var n = CountPrecedingParagraphsWithListNumLegalDefault(p, l) + 1;
            var num = n.ToString() + ".";
            for (var lAbove = l - 1; lAbove > 0; lAbove--)
            { // [2022] EWHC 3114 (Admin)
                var nAbove = CountPrecedingParagraphsWithListNumLegalDefault(p, lAbove) + 1;
        ...
        if (match.Success)
        {
            ...
            var n = CountPreceding(first, fieldCode) + start;
            var num = n.ToString() + ".";
        ...
        if (match.Success)
        {
            var numId = int.Parse(match.Groups[1].Value);
            var ilvl = int.Parse(match.Groups[2].Value) - 1;    // ilvl indexes are 0 based
            Paragraph p = first.Ancestors<Paragraph>().First();

            var n = Numbering3.CalculateN(main, p, ilvl);

            var fNum = DOCX.Numbering2.FormatNumber(numId, ilvl, n, main);
            RunProperties rProps = first.RunProperties;
        ...
        if (match.Success)
        {
            ...
            {
                var numId = p.ParagraphProperties.NumberingProperties.NumberingId.Val.Value;
                var ilvl = int.Parse(match.Groups[1].Value) - 1;    // ilvl indexes are 0 based
                var fNum = DOCX.Numbering2.FormatNumber(numId, ilvl, 1, main);
                RunProperties rProps = first.RunProperties;
                return new DOCX.WNumber2(fNum, rProps, main, p.ParagraphProperties);
            }
        }
        ...
        {
            var l = int.Parse(match.Groups[1].Value);
            var n = CountPreceding(first, fieldCode) + 1;
            var fNum = FormatNNumberDefault(l, n);
            ParagraphProperties pProps = first.Ancestors<Paragraph>().First().ParagraphProperties;
            RunProperties rProps = first.RunProperties;
        ...
        if (match.Success)
        {
            ...
            var n = CountPreceding(first, fieldCode) + s;
            var fNum = FormatNNumberDefault(l, n);
            ParagraphProperties pProps = first.Ancestors<Paragraph>().First().ParagraphProperties;
            RunProperties rProps = first.RunProperties;
            return new DOCX.WNumber2(fNum, rProps, main, pProps);
        ...
        if (match.Success)
        {
            ...
            var fNum = DOCX.Numbering2.FormatNumber(name, ilvl, start, main);
            ParagraphProperties pProps = first.Ancestors<Paragraph>().First().ParagraphProperties;
            RunProperties rProps = first.RunProperties;
            return new DOCX.WNumber2(fNum, rProps, main, pProps);
        }
        ...
        var fieldNum = CatchAllListNums(main, first, fieldCode);
    ...
    private static string CatchAllListNums(MainDocumentPart main, Run first, string fieldCode)
    {
        ...
        else if ((match = Regex.Match(fieldCode, @"\\l \((\d+)\)")).Success)
        {
            fieldLvl = int.Parse(match.Groups[1].Value) - 1; // fieldLvl is 0 based
        }
        var fieldStart = -1;
        ...
        else
        {
            if (fieldStart > -1)
            {
                var n = CountPreceding(first, fieldCode) + fieldStart;
                fieldNum = n.ToString() + ".";
            }
            else
            {
    ...
    {
        var test = " LISTNUM LegalDefault \\l " + l + " ";
        Func<Paragraph, bool> predicate = (p) =>
        {
            var fc = DOCX.Fields.ExtractAndNormalizeFieldCode(p);
            if (fc == test)
                return true;
